...
<?php if ( have_posts() ) : ?>
	...
	<div class="page-breadcrumb-heading">
		<h3 class="page-title">
					<?php if ( is_day() ) : ?>
						<?php printf( __( 'Academy %s', 'ppmlayout' ), '<small>' . get_the_date() . '</small>' ); ?>
					<?php elseif ( is_month() ) : ?>
						<?php printf( __( 'Academy %s', 'ppmlayout' ), '<small>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'ppmlayout' ) ) . '</small>' ); ?>
					<?php elseif ( is_year() ) : ?>
						<?php printf( __( 'Academy %s', 'ppmlayout' ), '<small>' . get_the_date( _x( 'Y', 'yearly archives date format', 'ppmlayout' ) ) . '</small>' ); ?>
					<?php else : ?>
						<?php _e( 'Academy', 'ppmlayout' ); ?>
					<?php endif; ?>
		</h3>
		<ul class="breadcrumb">
			<li>
	    ...
	    <div id="dashboard">
	       <div class="row-fluid">
	          <div class="span12 responsive">
				<?php ppmlayout_content_nav( 'nav-above' ); ?>
				<?php /* Start the Loop */ ?>
