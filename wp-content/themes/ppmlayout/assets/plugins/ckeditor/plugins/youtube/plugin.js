				...
				return {
					...
					onOk: function()
					{
						...
						if ( this.getContentElement( 'youtubePlugin', 'txtEmbed' ).isEnabled() )
						{
							...
							else if( wistia !== false ) {
								content = '<iframe src="'+wistia+'" frameborder="0" allowfullscreen width="'+width+'" height="'+height+'"></iframe>';
							}
							else {
...
{
	var p = /^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|wistia|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/;
	
	var e = 'wistia';
	
	//Check to see if it's a wistia or youtube video.
	if( url.match(p) ) {
		return true;
	}
	else if( url.match(e) ) {
	...
	//return ( url.match( p )|| url.match(e) ) ? RegExp.$1 : false;
}
