...
function check_cdn() {
	...
	// Check if CDN exists
	try {
		$cdn_instance = (isset($cdn_instance) &&!is_null($cdn_instance) && is_object($cdn_instance)) ? $cdn_instance : new RS_CDN();
	} catch (Exception $exc) {
		return false;
	}
 ...
 */
function verify_exists( $file_name ) {
    global $cdn_instance;
    ...
    // Make sure we have just the basename, no path
    $file_name = basename($file_name);

    // Get object
    try {
        $cdn_instance->container_object()->DataObject($file_name);
    } catch (Exception $exc) {
        return false;
    }
