		...
		foreach($results as $result) {
			...
			if (!$p_in_c) {
				...
				if ($show_author && $show_date) {
				   ...
				   $output .='<a href="'.get_permalink($result->ID).'"  '.$rel_attribute.' '.$target_attribute.' class="btn">';
	               $output .='<span>'.$title.'</span>';
	               $output .='<em><i class="icon-calendar"></i> '.mysql2date(get_option('date_format','d/m/y'), $result->post_date).'</em>';
	               $output .='<em><i class="icon-user"></i> ';
	               $output .= $author_name;
	               $output .='</em>';
	               $output .='<i class="icon-briefcase top-news-icons"></i>';
	               $output .='</a>';
				}
				else {
