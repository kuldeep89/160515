...
class WC_Cart {
	...
	public function __construct() {
		...
		$this->has_quoted_item = (isset($this->has_quoted_item)) ? $this->has_quoted_item : false;
		...
		public function calculate_totals() {
			...
			if ( sizeof( $this->cart_contents ) > 0 ) {
				foreach ( $this->cart_contents as $cart_item_key => $values ) {
					...
					$row_base_price 		= (strtolower(get_field('product_purchase_type', $_product->id)) == 'quote') ? 0 : $row_base_price;
			...
			if ( $this->prices_include_tax ) {
				...
				if ( sizeof( $this->cart_contents ) > 0 ) {
					foreach ( $this->cart_contents as $cart_item_key => $values ) {
						...
						$base_price 				= $_product->get_price();
						$base_price 				= (strtolower(get_field('product_purchase_type', $_product->id)) == 'quote') ? 0 : $base_price;
