...
<div id="order_review">
	...
	<table class="shop_table">
		...
		<tfoot>
			<?php if ($woocommerce->cart->has_quoted_item == false) : ?>
			<tr class="cart-subtotal">
				<th><?php echo ($woocommerce->cart->has_quoted_item == false) ? 'Cart' : 'Quote' ?> Subtotal</th>
				<td><?php echo $woocommerce->cart->get_cart_subtotal(); ?></td>
			</tr>
			<?php endif; ?>

			<?php if ( $woocommerce->cart->get_discounts_before_tax() && $woocommerce->cart->has_quoted_item == false ) : ?>

			<tr class="discount">
				<th><?php echo ($woocommerce->cart->has_quoted_item == false) ? 'Cart' : 'Quote' ?> Discount</th>
				<td>-<?php echo $woocommerce->cart->get_discounts_before_tax(); ?></td>
			</tr>

			<?php endif; ?>

			<?php if ( $woocommerce->cart->needs_shipping() && $woocommerce->cart->show_shipping() && $woocommerce->cart->has_quoted_item == false ) : ?>
			...
			<?php
				...
				if ( $woocommerce->cart->tax_display_cart == 'excl' && $woocommerce->cart->has_quoted_item == false ) {
			...
			<?php if ( $woocommerce->cart->get_discounts_after_tax() && $woocommerce->cart->has_quoted_item == false ) : ?>

			<tr class="discount">
				<th><?php echo ($woocommerce->cart->has_quoted_item == false) ? 'Order' : 'Quote' ?> Discount</th>
			...
			<?php if ($woocommerce->cart->has_quoted_item == false) : ?>
				<tr class="total">
					<th><strong><?php echo ($woocommerce->cart->has_quoted_item == false) ? 'Order' : 'Quote' ?> Total</strong></th>
					<td>
				...
				if (sizeof($woocommerce->cart->get_cart())>0) :
					foreach ($woocommerce->cart->get_cart() as $cart_item_key => $values) :
						...
						if ($_product->exists() && $values['quantity']>0) :
							...
							$product_price = (strtolower(get_field('product_purchase_type', $_product->id)) == 'quote') ? '<em>Quoted</em>' : apply_filters( 'woocommerce_checkout_item_subtotal', $woocommerce->cart->get_product_subtotal( $_product, $values['quantity'] ), $values, $cart_item_key );
							
							echo '<td class="product-total">' . $product_price . '</td>
								</tr>';
						endif;
					endforeach;
				endif;
	...
	<div id="payment">
		<?php if ($woocommerce->cart->needs_payment() && $woocommerce->cart->has_quoted_item == false) : ?>
