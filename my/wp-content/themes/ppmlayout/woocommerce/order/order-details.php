...
if (sizeof($order->get_items())>0) {
	foreach($order->get_items() as $item) {
		if (strtolower(get_field('product_purchase_type', $item['product_id'])) == 'quote') {
			$quoted_items[] = $item;
		}
	}
}
?>
<h2><?php echo (count($quoted_items) > 0) ? 'Quote Details' : 'Order Details' ?></h2>
	...
	<tbody>
		...
		if (sizeof($order->get_items())>0) {
			...
			foreach($order->get_items() as $item) {
				...
				// Check if quoted or not, don't display "You will be billed on..." unless this item is a subscription
				if (strtolower(get_field('product_purchase_type', $item['product_id'])) == 'quote') {
					echo '</td><td class="product-total"><em>Quoted</em></td></tr>';
				} else {
					if (trim($order->recurring_payment_method) !== '') {
						echo '</td><td class="product-total">' . $order->get_formatted_line_subtotal( $item ) . ' '.'( You will be billed on '.$date.' )'.'</td></tr>';
					} else {
