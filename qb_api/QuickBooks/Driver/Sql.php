	...
	*/
	...
	protected function _oauthLoad($app_username, $app_tenant)
	{
		...
		if ($arr = $this->fetch($this->query("
			...
			FROM 
				" . $this->_mapTableName(QUICKBOOKS_DRIVER_SQL_OAUTHTABLE) . " 
			WHERE
				merchant_id = '%s' AND app_tenant = '%s' ", $errnum, $errmsg, null, null, array( $merchant_id, $app_tenant ))))
		{
			$this->query("
				UPDATE 
					" . $this->_mapTableName(QUICKBOOKS_DRIVER_SQL_OAUTHTABLE) . "
				SET
					touch_datetime = '%s' 
				WHERE
					merchant_id = '%s' AND app_tenant = '%s' ", $errnum, $errmsg, 0, 1, array( date('Y-m-d H:i:s'), $merchant_id, $app_tenant ));
		...
		if ($arr = $this->_oauthLoad($app_username, $app_tenant))
		{
			...
			return $this->query("
				...
				SET 
					...
					request_datetime = '%s'
				WHERE
					merchant_id = '%s' AND app_tenant = '%s' ", $errnum, $errmsg, null, null, array( $token, $token_secret, date('Y-m-d H:i:s'), $merchant_id, $app_tenant ));
		}
		else
		{
			...
			return $this->query("
				...
				(
					...
					'%s',
					'%s'
				)", $errnum, $errmsg, null, null, array( $app_username, $app_tenant, $token, $token_secret, $merchant_id, date('Y-m-d H:i:s') ));
		}
	}
	...
	protected function _oauthAccessDelete($app_username, $app_tenant)
	{
		...
		$this->query("
			...
			WHERE
			merchant_id = '%s' AND 
			app_tenant = '%s' ", $errnum, $errmsg, null, null, array( $merchant_id, $app_tenant ));
	...
	protected function _initialize($init_options = array())
	{
		...
		*/
		...
		$def = array(
			...
			'merchant_id' => array( QUICKBOOKS_DRIVER_SQL_VARCHAR, 32, 'null' ), 	
