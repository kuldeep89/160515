...
intuit.ipp.anywhere.controller = {
    ...
    getOAuthUri: function () {
        ...
        var oauthurl = "https://" + intuit.ipp.anywhere.serviceHost + "/Connect/SessionStart?grantUrl=" + encodeURIComponent(intuit.ipp.anywhere.grantUrl);

        var datasources = [];
        if (intuit.ipp.anywhere.datasources.payments) {
            datasources.push("payments");
        }
        if (intuit.ipp.anywhere.datasources.quickbooks) {
            datasources.push("quickbooks");
        }

        var paymentOptions = [];

        if (intuit.ipp.anywhere.paymentOptions) {

            if (!intuit.ipp.anywhere.paymentOptions ||
	            !intuit.ipp.anywhere.paymentOptions.intuitReferred) {
                paymentOptions.push("intuitreferred=false");
            } else {
                paymentOptions.push("intuitreferred=true");
            }
        }
        ...
        oauthurl += "&datasources=" + encodeURIComponent(datasources.join()) + "&" + paymentOptions.join('&');
