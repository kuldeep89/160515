...
function get_transactional_data() {
	...
	if (is_null($merchant_id)) {
		...
		//6june2015: Chetu: Get QuickBook Cash Data
        $qb_data = $wpdb->get_results("SELECT card_type,amt,transaction_time FROM  ".$wpdb->prefix."ppttd_cashtransactions WHERE merchant_id='".$merchant_id."' AND transaction_time BETWEEN '".$start_date."' AND '".$end_date."'");
		
		foreach ($qb_data as $qb_trans) {
			$qb_trans->amt = number_format($qb_trans->amt, 2);
			$qb_trans->card_lastfour = '';
			$transactional_data[] = $qb_trans;
			if (!in_array($qb_trans->card_type, $card_types)) {
					array_push($card_types, $qb_trans->card_type);
			}
		}
	}
