<?php
	...
	* Date: 26 June 2013
	* Notes:
	* This controller handles operations of the academy module.
	*
	*/
	...
	class Academy extends MY_Controller {
		...
		**/
		public function ajax_entries() {
			...
			$obj_entries	= $this->academy_model->get_entries();
			...
			// Load JSON View
			////////////////
			$this->load->view('backend/object-templates/academy/json-entries', $arr_page);
		...
		**/
		public function add_tag() {
			...
			$this->load->view('backend/pages/academy/add-tag');
		...
		**/
		public function add_category() {
			...
			$this->load->view('backend/pages/academy/add-category');
		...
		**/
		public function category( $category_id = FALSE ) {
			...
			$obj_entry_collection	= $this->academy_model->get_categorized_entries($category_id);
			...
			// Load View
			////////////////
			$this->load->view('backend/pages/academy/category-listing', $arr_page);
		...
		**/
		public function create_entry() {
			...
			$this->load->model('academy_model');
			$obj_entry	= new Academy_entry_class();
			...
			$entry_id	= $this->academy_model->add_entry($obj_entry);
			
			////////////////
			// Now redirect the user to the edit page.
			////////////////
			redirect('academy/entry/'.$entry_id);
		...
		**/
		public function edit_category( $category_id ) {
			...
			$arr_category	= $this->academy_model->get_categories();
			
			if( count($arr_category) < 1 ) {
				
				$this->notification_lib->add_error('Invalid category ID, please try again.');
				redirect('academy/manage-categories');
				die();
			...
			// Load View
			////////////////
			$this->load->view('backend/pages/academy/edit-category', $arr_page);
		...
		**/
		public function edit_tag( $tag_id ) {
			...
			$arr_tag	= $this->academy_model->get_tags();
			
			if( count($arr_tag) < 1 ) {
				
				$this->notification_lib->add_error('Invalid tag ID, please try again.');
				redirect('academy/manage-tags');
				die();
			...
			// Load View
			////////////////
			$this->load->view('backend/pages/academy/edit-tag', $arr_page);
		...
		**/
		...
		public function get_breadcrumbs( $arr_params ) {
			...
			$breadcrumbs[]	= array('title'=> 'Academy', 'url' => site_url('academy'));

			switch( $method ) {
				
				case 'entry':
					$breadcrumbs[]	= array('title'=> 'Entry', 'url' => site_url('academy'));
					$breadcrumbs[]	= array('title'=> $arr_params['item_name'], 'url' => site_url('academy/entry/'.$arr_params['item_id']));
					return $breadcrumbs;
				break;
				case 'category':
					$breadcrumbs[]	= array('title'=> 'Category', 'url' => site_url('academy'));
					$breadcrumbs[]	= array('title'=> $arr_params['item_name'], 'url' => site_url('academy/category/'.$arr_params['item_id']));
					return $breadcrumbs;
				break;
				case 'listing':
					$breadcrumbs[]	= array('title'=> 'Entries', 'url' => site_url('academy/'.str_replace("_","-",$method)));
					return $breadcrumbs;
				case 'manage_categories':
					$breadcrumbs[]	= array('title'=> 'Manage Entry Categories', 'url' => site_url('academy/'.str_replace("_","-",$method)));
					return $breadcrumbs;
				case 'manage_tags':
					$breadcrumbs[]	= array('title'=> 'Manage Entry Tags', 'url' => site_url('academy/'.str_replace("_","-",$method)));
					return $breadcrumbs;
				default:
					return $breadcrumbs;
					break;
		...
		**
		**	Description:
		**	Default Academy page, shows recent academy entries.
		**
		**	@param:		void
		**	@return:	void
		**
		...
		**/
		public function index() {
			...
			$obj_entry_collection	= $this->academy_model->get_entries(25);
			...
			$this->load->view('backend/pages/academy/entry-collection-feed', $arr_page);
		...
		**/
		public function entry( $entry_id ) {
			...
			// Get Related Entry
			////////////////
			$obj_related_collection	= $this->academy_model->get_categorized_entries($obj_entry->get_categories(), $obj_entry->get('id'), 4);
			...
			////////////////
			$arr_tags		= $this->academy_model->get_tags();
			$arr_categories	= $this->academy_model->get_categories();
			...
			$arr_page['arr_js_views']			= array('backend/object-templates/academy/js-entry-tags', 'backend/object-templates/academy/js-entry-categories');
			$arr_page['obj_author_collection']	= $obj_author_collection;
			$arr_page['browser_title']			= (isset($browser_title) && !empty($browser_title))? $browser_title.' - MyBizPerks Academy':$obj_entry->get('title').' - MyBizPerks Academy';
			...
			// Load the View
			////////////////
			$this->load->view('backend/pages/academy/entry', $arr_page);
		...
		**/
		public function insert_category() {
			...
			if( empty($category) ) {
				...
				redirect('academy/add-category');
				
			}
			
			$this->load->model('academy_model');
			
			$this->academy_model->add_category($category, $description, $color, $icon);
		...
		**/
		public function insert_tag() {
			...
			if( empty($tag) ) {
				...
				redirect('academy/add-tag');
				
			}
			
			$this->load->model('academy_model');
			
			$this->academy_model->add_tag($tag, $description);
		...
		**
		**	Description:
		**	This method lists the academy entries that have been
		**  added to the system.
		**
		**	@param:		void
		**	@return:	void
		...
		**/
		public function listing() {
			...
			////////////////
			$this->load->model('academy_model');
			$obj_entry_collection	= $this->academy_model->get_entries(10000, FALSE, TRUE);
			...
			$this->load->view('backend/pages/academy/entries', $arr_page);
		...
		**/
		public function remove_category( $category_id ) {
			...
			////////////////
			$num_entries	= $this->academy_model->get_num_entries_in_categories($category_id);
			
			if( $this->security_lib->permissions_required(10) || $this->security_lib->permissions_required(11) && $num_entries < 1 ) {
			
				$this->academy_model->remove_category($category_id);
		...
		**/
		public function remove_tag( $tag_id ) {
			...
			$this->load->model('academy_model');
			$this->academy_model->remove_tag($tag_id);
		...
		**/
		public function tag( $tag_id ) {
			...
			$obj_entry_collection	= $this->academy_model->get_tagged_entries($tag_id);
			...
			// Load View
			////////////////
			$this->load->view('backend/pages/academy/tag-listing', $arr_page);
		...
		**/
		public function manage_categories() {
			...
			$arr_category_entries	= $this->academy_model->get_num_entries_in_categories($arr_categories);
			...
			$this->load->view('backend/pages/academy/manage-categories', $arr_page);
		...
		**/
		public function manage_tags() {
			...
			$arr_tag_entries	= $this->academy_model->get_num_entries_in_tags($arr_tags);
			...
			$this->load->view('backend/pages/academy/manage-tags', $arr_page);
		...
		**/
		public function update_category( $category_id ) {
			...
			////////////////
			$arr_category['academy_entry_category_id']	= $category_id;
			...
			$this->academy_model->update_category($arr_category);
		...
		**  the academy entry associated with it.
		**
		**	@param:		$entry_id
		**	@return:	void
		**
		...
		**/
		public function update_entry( $entry_id ) {
			...
			if( isset($arr['entry_id']) && is_numeric($arr['entry_id']) ) {
				$obj_entry	= $this->academy_model->get_entry($arr['entry_id']);
			}
