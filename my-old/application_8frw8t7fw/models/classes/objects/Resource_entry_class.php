<?php
	...
	* with a resource Entry.
	*
	*/
	
	require_once dirname(dirname(__FILE__)).'/standard/class.php';
	
	class Resource_entry_class extends Standard_class {
		
		////////////////
		// Object Data Members
		////////////////
		protected $id;
		protected $resource_entry_id;
		protected $author_id;
		...
		protected $keywords;
		protected $initial;
		protected $schedule_post;
		protected $schedule_remove;
		protected $last_user_modified;
		...
		**/
		public function get_categories( $force_reload = FALSE ) {
			...
			$this->arr_categories	= $this->CI->resource_model->get_entry_categories($this->id);
		...
		public function get_category_ids() {
			...
			foreach( $arr_categories as $arr_category ) {
				$arr_ids[$arr_category['resource_entry_category_id']] = $arr_category['resource_entry_category_id'];
			}
		...
		**/
		public function get_tags( $force_reload = FALSE ) {
			...
			$this->arr_tags	= $this->CI->resource_model->get_entry_tags($this->id);
		...
		**/
		public function save() {
			$this->CI->resource_model->update_entry($this);
		}
		...
		**/
		public function save_categories() {
			...
			if( count($this->arr_update_categories) > 0 ) {
				...
				////////////////
				$this->CI->db->delete('resource_entry_categories_assoc', 'resource_entry_id = '.$this->get('id'));
				
				////////////////
				// Get All Categories
				////////////////
				$arr_categories	= $this->CI->resource_model->get_categories();
				$arr_names		= array();
				...
				////////////////
				foreach( $arr_categories as $arr_category ) {
					$arr_names[$arr_category['resource_entry_category_id']]	= $arr_category['name'];
				}
				...
				foreach( $this->arr_update_categories as $category ) {
					...
					if( in_array($category, $arr_names) ) {
						$this->CI->db->insert('resource_entry_categories_assoc', array('resource_entry_id' => $this->get('id'), 'resource_entry_category_id' => $arr_category_ids[$category]));
					}
					else {
						...
						////////////////
						$this->CI->db->insert('resource_entry_categories_assoc', array('resource_entry_id' => $this->get('id'), 'resource_entry_category_id' => $this->CI->resource_model->add_category($category)));
		...
		**/
		public function save_tags() {
			...
			if( count($this->arr_update_tags) > 0 ) {
				...
				////////////////
				$this->CI->db->delete('resource_entry_tags_assoc', 'resource_entry_id = '.$this->get('id'));
				
				////////////////
				// Get All Tags
				////////////////
				$arr_tags	= $this->CI->resource_model->get_tags();
				$arr_names	= array();
				...
				foreach( $arr_tags as $arr_tag ) {
					$arr_names[$arr_tag['resource_entry_tag_id']]	= $arr_tag['name'];
				}
				...
				foreach( $this->arr_update_tags as $tag ) {
					...
					if( in_array($tag, $arr_names) ) {
						$this->CI->db->insert('resource_entry_tags_assoc', array('resource_entry_id' => $this->get('id'), 'resource_entry_tag_id' => $arr_tag_ids[$tag]));
					}
					else {
						...
						////////////////
						$this->CI->db->insert('resource_entry_tags_assoc', array('resource_entry_id' => $this->get('id'), 'resource_entry_tag_id' => $this->CI->resource_model->add_tag($tag)));
		...
		**
		**/
		public function set_resource_entry_id( $id ) {
		
			$this->resource_entry_id	= $id;
			$this->id				= $id;
