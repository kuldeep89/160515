<?php
	...
	* add/update/remove data associated with the resource model.
	*
	*/
	
	require_once dirname(__FILE__).'/classes/collections/Resource_entries_collection.php';
	require_once dirname(__FILE__).'/classes/objects/Resource_entry_class.php';
	...
	class resource_model extends CI_Model {
		...
		**/
		public function add_category( $name, $description = FALSE, $color = FALSE, $icon = FALSE ) {
			...
			$this->db->insert('resource_entry_categories', $arr_data);
		...
		**/
		public function add_entry( $obj_entry ) {
			...
			////////////////
			$entry_id	= $obj_entry->get('resource_entry_id');
			
			if( isset($entry_id) && $entry_id !== FALSE ) {
				$arr_fields[]	= 'resource_entry_id';
			}
			...
			$this->db->insert('resource_entries', $arr_data);
		...
		**/
		public function add_tag( $name, $description = FALSE ) {
			...
			$this->db->insert('resource_entry_tags', $arr_data);
			return $this->db->insert_id();	
		...
		**/
		public function get_categories( $force_reload = FALSE ) {
			...
			$obj_query	= $this->db->get('resource_entry_categories');
			$arr_rows	= $obj_query->result_array();
			
			foreach( $arr_rows as $arr_row ) {
				$this->arr_all_categories[$arr_row['resource_entry_category_id']]	= $arr_row;
			}
		...
		**/
		public function get_entries( $limit = 999, $force_reload = FALSE, $all_entries = FALSE ) {
			...
			$obj_query				= $this->db->get('resource_entries', $limit, 0);
			
			$obj_entry_collection	= new resource_entries_collection();
			...
			if( $obj_query->num_rows() > 0 ) {
				...
				foreach($arr_rows as $arr_row ) {
					...
					$obj_entry_collection->add(new resource_entry_class($arr_row));
		...
		**/
		public function get_entry( $entry_id ) {
			...
			// Get Entry
			////////////////
			$this->db->where('resource_entry_id', $entry_id);
			
			$obj_query	= $this->db->get('resource_entries');
			...
			if( $obj_query->num_rows() > 0 ) {
				...
				$arr_rows	= $obj_query->result_array();
				$obj_entry	= new resource_entry_class(array_pop($arr_rows));
		...
		**/
		public function get_entries_like( $search ) {
			...
			$obj_query				= $this->db->get('resource_entries', 10);
			$obj_entry_collection	= new resource_entries_collection();
		...
		**/
		public function get_entry_tags( $entry_id ) {
			...
			////////////////
			$this->db->where('resource_entry_id', $entry_id);
			$this->db->join('resource_entry_tags', 'resource_entry_tags.resource_entry_tag_id = resource_entry_tags_assoc.resource_entry_tag_id', 'left');
			
			$obj_query	= $this->db->get('resource_entry_tags_assoc');
		...
		**/
		public function get_entry_categories( $entry_id ) {
			...
			////////////////
			$this->db->where('resource_entry_id', $entry_id);
			$this->db->join('resource_entry_categories', 'resource_entry_categories.resource_entry_category_id = resource_entry_categories_assoc.resource_entry_category_id', 'left');
			
			$obj_query	= $this->db->get('resource_entry_categories_assoc');
		...
		**/
		public function get_categorized_entries( $arr_categories, $omit_entry_id = FALSE, $limit = FALSE ) {
			...
			$this->db->distinct('resource_entry_categories_assoc.resource_entry_id');
			$this->db->select('resource_entries.*');
			$this->db->where('resource_entries.published', 1);
			////////////////
			...
			if( count($arr_categories) > 0 ) {
				...
				foreach( $arr_categories as $arr_category ) {
					...
					$category_id	= (is_array($arr_category))? $arr_category['resource_entry_category_id']:$arr_category;
					
					if( $first ) {
						
						$where	= '(resource_entry_categories_assoc.resource_entry_category_id = '.$category_id;
						$first = FALSE;
						
					}
					else {
						$where .= ' OR resource_entry_categories_assoc.resource_entry_category_id = '.$category_id;
					}
			...
			if( $omit_entry_id !== FALSE ) {
				$this->db->where('resource_entry_categories_assoc.resource_entry_id != '.$omit_entry_id);
			}
			
			$this->db->join('resource_entries', 'resource_entry_categories_assoc.resource_entry_id = resource_entries.resource_entry_id');
			$obj_query	= $this->db->get('resource_entry_categories_assoc', $limit, 0);
		...
		**/
		public function get_num_entries_in_categories( $arr_categories ) {
			...
			if( !is_array($arr_categories) ) {
				$arr_categories	= array(array('resource_entry_category_id' => $arr_categories));
			}
			...
			foreach( $arr_categories as $arr_category ) {
				...
				$this->db->where('resource_entry_category_id', $arr_category['resource_entry_category_id']);
				$arr_rtn[$arr_category['resource_entry_category_id']]	= $this->db->count_all_results('resource_entry_categories_assoc');
		...
		**/
		public function get_num_entries_in_tags( $arr_tags ) {
			...
			foreach( $arr_tags as $tag ) {
				...
				$this->db->where('resource_entry_tag_id', $tag['resource_entry_tag_id']);
				$arr_rtn[$tag['resource_entry_tag_id']]	= $this->db->count_all_results('resource_entry_tags_assoc');
		...
		**/
		public function get_tagged_entries( $tag, $omit_entry_id = FALSE ) {
			...
			$this->db->distinct('resource_entry_tags_assoc.resource_entry_id');
			$this->db->select('resource_entries.*');
			$this->db->where('resource_entries.published', 1);
			////////////////
			...
			if( count($arr_tags) > 0 ) {
				...
				foreach( $arr_tags as $tag ) {
					...
					if( $first ) {
						...
						$where	= '(resource_entry_tags_assoc.resource_entry_tag_id = '.$tag;	
						$first	= FALSE;
						
					}
					else {
						$where .= ' OR resource_entry_tags_assoc.resource_entry_tag_id = '.$tag;
					}
			...
			if( $omit_entry_id !== FALSE ) {
				$this->db->where('resource_entry_tags_assoc.resource_entry_id != '.$omit_entry_id);
			}
			
			$this->db->join('resource_entries', 'resource_entry_tags_assoc.resource_entry_id = resource_entries.resource_entry_id');
			$obj_query	= $this->db->get('resource_entry_tags_assoc');
		...
		**/
		public function get_tags( $force_reload = FALSE ) {
			...
			$obj_query			= $this->db->get('resource_entry_tags');
		...
		**/
		public function remove_category( $category_id ) {
			...
			$this->db->delete('resource_entry_categories');
			$this->db->where('resource_entry_category_id', $category_id);
			$this->db->delete('resource_entry_categories_assoc');
		...
		**/
		public function remove_entry( $entry_id ) {
			...
			if( $this->security_lib->accessible(3) ) {
				...
				$this->db->where('resource_entry_id', $entry_id);
				$this->db->delete('resource_entries');
				$this->db->where('resource_entry_id', $entry_id);
				$this->db->delete('resource_entry_categories_assoc');
				$this->db->where('resource_entry_id', $entry_id);
				$this->db->delete('resource_entry_tags_assoc');
		...
		**/
		public function remove_tag( $tag_id ) {
			...
			$this->db->delete('resource_entry_tags');
			$this->db->where('resource_entry_tag_id', $tag_id);
			$this->db->delete('resource_entry_tags_assoc');
		...
		**
		**	Description:
		**	This method updates the resource category.
		**
		**	@param:		$arr_category
		**	@return:	void
		**
		**/
		public function update_category($arr_category) {
			...
			unset($arr_category['resource_entry_category_id']);
			
			//Update table.
			$this->db->update('resource_entry_categories', $arr_category);
		...
		**
		**	Description:
		**	This method updates the resource tag.
		**
		**	@param:		$arr_tag
		**	@return:	void
		**
		**/
		public function update_tag($arr_tag) {
			...
			$this->db->where('resource_entry_tag_id', $arr_tag['resource_entry_tag_id']);
		
			//Remove id from array.
			unset($arr_tag['resource_entry_tag_id']);
			
			//Update table.
			$this->db->update('resource_entry_tags', $arr_tag);
		...
		**/
		public function update_entry( $obj_entry ) {
			...
			// Now Insert into DB
			////////////////
			$this->db->update('resource_entries', $arr_data, 'resource_entry_id = '.$obj_entry->get('id'));
